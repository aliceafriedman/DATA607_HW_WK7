---
title: "Loading Tables from HTML, JSON, and AJAX"
author: "Alice Friedman"
date: "October 9, 2018"
output:
  prettydoc::html_pretty:
    theme: hpstr
    highlight: vignette
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

##Setup
```{r, results="hide", message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(prettydoc)
library(rvest)
library(dplyr)
library(jsonlite)
remove(list=ls())
```

##HTML
First, store the raw HTML file as an R object using the ```read_html``` function.
```{r}
books.html <- read_html("https://raw.githubusercontent.com/aliceafriedman/DATA607_HW_WK7/master/books.html")
```

Then, parse the R object, ```html```, using the ```html_nodes``` function from the ```rvest``` library.
```{r}
books.html.table <- html_nodes(books.html, "table")
books.html.table <- html_table(books.html.table[1], fill = TRUE) %>% as.data.frame() %>% glimpse()
books.html.table %>% 
#Rename columsn to fit
  rename(Auth1.Last = First.Author.Last.Name,
         Auth1.First = First.Author.First.Name,
         Auth2.Last = Second.Author.Last.Name,
         Auth2.First = Second.Author.First.Name,
         Year = Year.Published) %>% 
  kable() %>% kable_styling(bootstrap_options = "responsive")
```

##JSON
First, read, the JSON obects into R.

```{r}
books.json <- fromJSON("https://raw.githubusercontent.com/aliceafriedman/DATA607_HW_WK7/master/books.json")
books.json
```


##AJAX